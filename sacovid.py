...
SA_URL = (
    "https://opendata.arcgis.com/datasets/48667a23f3b7468d8cd91afce7a6d047_0.geojson"
)
...
chart_dict = {
    ...
    "Ventilator Information": (
        ...
        "strac_covid_positive_on_ventila",
        "strac_total_ventilators",
        "strac_available_ventilators",
    ),
    "Staffed Bed Availability": (
        "Staffed Bed Availability Information",
        "strac_total_staffed_beds",
...
preset_dict = {
    "Daily Snapshot": [
        "Reported Cases 7dMA",
        "Daily Mortality 7dMA",
        "Daily COVID ICU Census",
        "Daily COVID Ventilator Census",
    ],
}
...
def fetch_state(state_abbrev):
    ...
    df = pd.read_json(
        f"https://covidtracking.com/api/v1/states/{state_abbrev.lower()}/daily.json"
    )
    ...
    df["testPositivity_7dMA"] = (
        df["positiveIncrease"] / df["totalTestResultsIncrease"]
    ).rolling(7).mean() * 100
    return df
...
def make_state_graphs(state_dict, state_graph_types, start_date, end_date):
    ...
    for graph_desc in graph_descs:
        for state, df in state_dict.items():
            df[state_graph_types[graph_desc]].loc[
                start_date : end_date + timedelta(days=1)
            ].plot(label=state.upper(), title=graph_desc, figsize=(8, 6))
        if graph_desc == "Test Positivity Rate 7d Moving Average(%)":
            plt.ylim(0, 100)
            plt.xlabel(
                "Data are not likely reliable on earlier dates when testing was less available."
            )
        else:
            plt.xlabel("Date")
...
def make_sa_chart(df, choice, start_date, end_date):
    if choice in ["Multiview"]:
        ...
        st.subheader(
            "This is a unique type of chart which allows overlay of multiple graphs."
        )
        ...
        if len(multi_choice) > 0:
            ax = (
                df[[multiview_options[x] for x in multi_choice]]
                .loc[start_date : end_date + timedelta(days=1)]
                .plot(title="Multiview Chart")
            )
        ...
        st.header(chart_dict[choice])
        st.subheader(
            "These charts allow comparison between different states using raw and population-adjusted data."
        )
        state_names = st.multiselect(
            "Select states to display:",
            options=state_pops["State"].tolist(),
            default=state_pops["State"].tolist()[:4],
        )
        if len(state_names) > 0:
            state_abbrevs = state_pops.index[
                state_pops["State"].isin(state_names)
            ].tolist()
            state_dict = {}
        ...
        ax1.set_ylabel("")
        ax2.set_ylabel(
            "Test Positivity Rate 7dMA (%)", rotation=270, labelpad=10, c="gray"
        )
        ax2.tick_params(axis="y", colors="gray")
        ax2.set_ylim(0, 100)
        df[df["BCLabTests"].notnull()]["BCLabTests"].loc[
            start_date : end_date + timedelta(days=1)
        ].plot(kind="area", label="Total Tests", title="Cumulative " + choice, ax=ax1)
        df[df["BCTestNegative"].notnull()]["BCTestNegative"].loc[
            start_date : end_date + timedelta(days=1)
        ].plot(kind="area", label="Negative Tests", ax=ax1)
        df[df["BCTestPositive"].notnull()]["BCTestPositive"].loc[
            start_date : end_date + timedelta(days=1)
        ].plot(kind="area", label="Positive Tests", ax=ax1)
        df["TestPositivityRate"].loc[
            start_date : end_date + timedelta(days=1)
        ].abs().plot(label="Test Positivity Rate", c="white", ax=ax2)
        ax1.legend()
        st.pyplot()
        ...
        ax2.tick_params(axis="y", colors="gray")
        ax2.set_ylim(0, 100)
        df[df["DBCLabTests"].notnull()]["DBCLabTests"].loc[
            start_date : end_date + timedelta(days=1)
        ].plot(kind="area", label="Total Tests", title="Daily " + choice, ax=ax1)
        df[df["DBCTestNegative"].notnull()]["DBCTestNegative"].loc[
            start_date : end_date + timedelta(days=1)
        ].plot(kind="area", label="Negative Tests", ax=ax1)
        df[df["DBCTestPositive"].notnull()]["DBCTestPositive"].loc[
            start_date : end_date + timedelta(days=1)
        ].abs().plot(kind="area", label="Positive Tests", ax=ax1)
        df["TestPositivityRate"].loc[
            start_date : end_date + timedelta(days=1)
        ].abs().plot(label="Test Positivity Rate", c="white", ax=ax2)
        ax1.legend()
        ...
        df["DBCTestPositive7dMA"].loc[start_date : end_date + timedelta(days=1)].plot(
            label="Daily Positive Tests 7d Moving Avg",
            title="Reported Cases and Daily Postive Tests",
        )
        df["Reported7dMA"].loc[start_date : end_date + timedelta(days=1)].plot(
            label="Daily Reported Cases 7d Moving Avg"
        )
        df[df["DBCTestPositive"].notnull()]["DBCTestPositive"].loc[
            start_date : end_date + timedelta(days=1)
        ].abs().plot(kind="area", label="Daily Positive Tests", alpha=0.2)
        plt.legend()
        st.pyplot()
        ...
        l1 = (
            df[chart_dict[choice][1]]
            .loc[start_date : end_date + timedelta(days=1)]
            .plot(
                kind="line",
                ax=ax2,
                c=RED,
                title=f"Daily and Cumulative {choice} With 7dMA",
            )
        )
        l2 = (
            df[chart_dict[choice][2]]
            .loc[start_date : end_date + timedelta(days=1)]
            .plot(kind="area", ax=ax1, alpha=0.3, label="_nolegend_")
        )
        l3 = (
            df[chart_dict[choice][3]]
            .loc[start_date : end_date + timedelta(days=1)]
            .plot(kind="line", ax=ax1, c=ORANGE, label=f"Daily {choice} 7d Moving Avg")
        )
        ...
        st.header(chart_dict[choice][0])
        fig, ax1 = plt.subplots()
        df[chart_dict[choice][1]].loc[start_date : end_date + timedelta(days=1)].plot(
            label="Cumulative Recoveries", title="Recoveries"
        )
        df[chart_dict[choice][2]].loc[start_date : end_date + timedelta(days=1)].plot(
            label="Daily Change in Recoveries"
        )
        plt.legend()
        st.pyplot()
        df[chart_dict[choice][3]].loc[start_date : end_date + timedelta(days=1)].plot(
            title="Patients Who Are Still Ill", c=RED
        )
        st.pyplot()
        ...
        st.header(chart_dict[choice][0])
        df[chart_dict[choice][1]].loc[start_date : end_date + timedelta(days=1)].plot(
            title=f"Daily COVID ICU Census"
        )
        st.pyplot()
        ...
        st.header(chart_dict[choice][0])
        df[chart_dict[choice][1]].loc[start_date : end_date + timedelta(days=1)].plot(
            title=f"Daily COVID Ventilator Census"
        )
        st.pyplot()
        df[chart_dict[choice][2]].loc[start_date : end_date + timedelta(days=1)].plot(
            kind="area", label="Total Ventilators", title="Ventilator Availability"
        )
        df[chart_dict[choice][3]].loc[start_date : end_date + timedelta(days=1)].plot(
            kind="area", label="Available Ventilators"
        )
        plt.legend(loc=2)
        st.pyplot()
        ...
        st.header(chart_dict[choice][0])
        df[chart_dict[choice][1]].loc[start_date : end_date + timedelta(days=1)].plot(
            kind="area", label="Total Staffed Beds"
        )
        df[chart_dict[choice][2]].loc[start_date : end_date + timedelta(days=1)].plot(
            kind="area", label="Available Staffed Beds"
        )
        plt.legend(loc=2)
        st.pyplot()
...
def build_site():
    ...
    st.markdown(
        f"The data for these charts were last updated on {str(sa_df.index.max()).split()[0]}. "
